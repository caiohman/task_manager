    ...
    <section class="flex-row" style="justify-content: space-evenly">
        ...
        <Fieldset :legend="atms" style="width: 30%; margin-left: 3%; height: fit-content" >
            <div class="flex-column">
                ...
                <InputFilter :field="models" :placeholder="selectModel" class="space-below"
                    :submitted="submitted" @select-option="(option) => validateField(option, field[1].id)"/>
                <InputNumber v-model="atmNumber" size="small" :placeholder= atm :useGrouping="false" class="space-below"/>
                <InputText v-model="coordinates" size="small" :placeholder= coordinate coordinate type="text" class="space-below"/>
	            <InputText v-model="addressName" size="small" :placeholder= name type="text" class="space-below" />

                <section class="options-line">
                    <Toast />
                    <Button icon="pi pi-plus"  @click="saveNewAtmValidation"/>
                </section>
            </div>
        </Fieldset>
    </section>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            return {
                t, selectedRegion, region, models, atmNumber, 
                    coordinates, selectedModel, submitted, toast, addressName 
            }
        ...
        data() {
            return {
                ...
                field: [{id: 1, name: 'region'}, {id: 2, name: 'models'}],
        ...
        methods: {
            ...
            validateField(value, field) {
                if(value === undefined || value.length === 0) {
                    ...
                    (field === this.field[0].id) ? this.selectedRegion = value : this.submitUpdate(value);
                }
            },
            ...
            async newAtm() {
                if(this.selectedRegion !== undefined && this.selectedModel !== undefined &&
                    this.atmNumber !== undefined && this.coordinates !== undefined &&
